...
const groupByMonth = (data: HistoricalDataPoint[]): BPDataPoint[] => {
  const monthMap = new Map<string, { sumSbp: number; sumDbp: number; count: number }>()

  data.forEach((point) => {
    // Filtrar valores 0 para no diluir el promedio
    if (point.systolic <= 0 || point.diastolic <= 0) return

    const date = new Date(point.date)
    const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`

    if (!monthMap.has(monthKey)) {
      monthMap.set(monthKey, { sumSbp: 0, sumDbp: 0, count: 0 })
    }
    const entry = monthMap.get(monthKey)!
    entry.sumSbp += point.systolic
    entry.sumDbp += point.diastolic
    entry.count += 1
  })
  ...
  return Array.from(monthMap.entries())
    ...
    .map(([key, value]) => {
      ...
      return {
        ...
        sbp: Math.round(value.sumSbp / value.count),
        dbp: Math.round(value.sumDbp / value.count),
      }
    })
}

export function BPChartNew({ historicalData, isLoadingHistory = false }: BPChartProps) {
  const [timeRange, setTimeRange] = useState<TimeRange>("7D")
  const [pmYear, setPmYear] = useState("2026")

  // Procesar datos según el rango seleccionado
  const data = useMemo<BPDataPoint[]>(() => {
    ...
    if (timeRange === "7D") {
      ...
      return filtered.map((point) => ({
        ...
        firestoreTimestamp: point.timestampBP,
      }))
    }
    ...
    if (timeRange === "1Y") {
      // 1Y: Mostrar una etiqueta por mes (máximo 12)
      const monthNames = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"]
      ...
      return filtered.map((point) => {
        ...
        // Mostrar etiqueta solo si es la primera vez que vemos este mes
        let labelText = ""
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between flex-wrap gap-2">
          <CardTitle className="text-sm font-semibold" style={{ fontSize: '14px', fontWeight: 600, color: '#111827' }}>Presión Arterial — Sistólica/Diastólica (mmHg)</CardTitle>
        ...
        <ResponsiveContainer width="100%" height="100%">
          {isPM ? (
            <BarChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              ...
              />
              <ReferenceLine y={90} stroke="hsl(210, 70%, 50%)" strokeDasharray="3 3" opacity={0.3} />
              <ReferenceLine y={120} stroke="hsl(210, 70%, 50%)" strokeDasharray="3 3" opacity={0.3} />
              <ReferenceLine y={60} stroke="hsl(142, 70%, 45%)" strokeDasharray="3 3" opacity={0.3} />
              <ReferenceLine y={80} stroke="hsl(142, 70%, 45%)" strokeDasharray="3 3" opacity={0.3} />
              <Tooltip
                    ...
                    return (
                      ...
                      >
                        ...
                        <p style={{ color: '#3b82f6' }}>
                          Sistólica: <strong>{data.sbp} mmHg</strong>
                        </p>
                        <p style={{ color: '#22c55e' }}>
                          Diastólica: <strong>{data.dbp} mmHg</strong>
                        </p>
                      </div>
                    )
                  }
              ...
              />
              <Bar dataKey="sbp" fill="hsl(210, 70%, 50%)" radius={[6, 6, 0, 0]} />
            </BarChart>
          ) : (
