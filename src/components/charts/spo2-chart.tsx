...
const groupByMonth = (data: HistoricalDataPoint[]): SpO2DataPoint[] => {
  const monthMap = new Map<string, { sum: number; count: number }>()

  data.forEach((point) => {
    // Filtrar valores 0 para no diluir el promedio
    if (point.spo2 <= 0) return
...
export function SpO2Chart({ historicalData, isLoadingHistory = false }: SpO2ChartProps) {
  const [timeRange, setTimeRange] = useState<TimeRange>("7D")
  const [pmYear, setPmYear] = useState("2026")

  // Procesar datos según el rango seleccionado
  const data = useMemo<SpO2DataPoint[]>(() => {
    ...
    if (timeRange === "7D") {
      ...
      return filtered
        .filter((point) => point.spo2 > 0) // ✅ Eliminar puntos con valor 0
        .map((point) => ({
          time: formatDayOfWeek(point.date),
          value: point.spo2,
          simulatedDate: point.date,
          firestoreTimestamp: point.timestampSpO2,
        }))
    }
    ...
    if (timeRange === "1M") {
      ...
      const monthNames = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"]
      const filteredNonZero = filtered.filter((point) => point.spo2 > 0) // ✅ Eliminar puntos con valor 0
    ...
    if (timeRange === "1Y") {
      ...
      const monthNames = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"]
      const filteredNonZero = filtered.filter((point) => point.spo2 > 0)
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between flex-wrap gap-2">
          <CardTitle className="text-sm font-semibold" style={{ fontSize: '14px', fontWeight: 600, color: '#111827' }}>Saturación O₂ — Histórico</CardTitle>
        ...
        <ResponsiveContainer width="100%" height="100%">
          {isPM ? (
            <BarChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              ...
              <YAxis domain={[94, 100]} tick={{ fontSize: 11 }} stroke="var(--muted-foreground)" />
              <ReferenceLine y={95} stroke="hsl(142, 70%, 45%)" strokeDasharray="3 3" opacity={0.3} />
              <ReferenceLine y={100} stroke="hsl(142, 70%, 45%)" strokeDasharray="3 3" opacity={0.3} />
              <Tooltip
